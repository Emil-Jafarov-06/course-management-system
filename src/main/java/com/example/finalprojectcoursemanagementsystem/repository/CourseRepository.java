...
public interface CourseRepository extends JpaRepository<Course, Long> {
    ...
    @Query(value = "SELECT c FROM Course c WHERE LOWER(c.courseDescription) LIKE LOWER(CONCAT('%', :courseDescription, '%'))",
            countQuery = "SELECT COUNT(c) FROM Course c WHERE LOWER(c.courseDescription) LIKE LOWER(CONCAT('%', :courseDescription, '%'))")
